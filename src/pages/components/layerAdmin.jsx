...
export default class extends React.Component {
    ...
    downloadTxtFile = () => {
        ...
        const { dialog } = remote;
        dialog.showErrorBox('Внимание', 'Начата отправка изменений на сервер')
    ...
    handleCommitChanges = async () => {
        ...
        const remote = window.require('electron').remote;
        const { dialog } = remote;
        dialog.showErrorBox('Внимание', 'Объект был сохранен')
    }
    ...
    handleDeletePoint = async () => {
        ...
        const remote = window.require('electron').remote;
        const { dialog } = remote;
        dialog.showErrorBox('Внимание', 'Объект был удален')
    }
    ...
    handleGoToMe = async () => {
        ...
        this.setState(update(this.state, {
            ...
            focusLocation: { $set: await person.information.getGEO() }
        }));
    }
    ...
    handleExchangeObject = async () => {
        ...
        if (document.getElementById('inputSearch').value == 'Монумент') {
            ...
            await this.openInfo(this.state.tempObj.idx);
            dialog.showErrorBox(messageText.title, messageText.body);
        }
    ...
    render() {
        ...
        return (
            <LeafletMap
                center={new Array(this.state.focusLocation.lat, this.state.focusLocation.long)}
                ...
                {(this.state.isYourLocation) ? <Marker
                    icon={iconYou}
                    position={new Array(this.state.focusLocation.lat, this.state.focusLocation.long)}
                >
                    <Popup fill="#000000">
                        <b><center>Ваша позиция</center></b>
                        <b>[{this.state.focusLocation.lat}, {this.state.focusLocation.long}]</b>
                    </Popup>
                </Marker> : null}
